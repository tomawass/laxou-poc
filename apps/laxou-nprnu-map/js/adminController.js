...
export class AdminController {
  ...
  showEditorModal(place = null, defaultX = 50.0, defaultY = 50.0) {
    ...
    document.getElementById("edit-place-category").value = place
      ? place.category
      : "services";
    document.getElementById("edit-place-color").value = place
      ? place.highlightColor || "#d8f0c1"
      : "#d8f0c1";
    ...
    document.getElementById("edit-place-address").value = place
      ? place.address
      : "Quartier des Provinces, 54520 Laxou";
    ...
    if (fileInput) {
      ...
      fileInput.onchange = (e) => {
        ...
        if (file) {
          ...
          reader.onload = (event) => {
            document.getElementById("edit-place-image").value =
              event.target.result;
          };
  ...
  savePlaceFromForm() {
    ...
    if (!name || !isFinite(x) || !isFinite(y)) {
      alert(
        "Veuillez remplir au moins le nom et les coordonnées valides (X% et Y% entre 0 et 100).",
      );
      return;
    }
