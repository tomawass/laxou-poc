...
const DEFAULT_FALLBACK_CONFIG = {
  ...
  garderie: {
    ...
    tarifsUniques: { occasionnel: 2.1, exterieur: 1.66 },
    mois: [
  ...
  restauration: {
    ...
    tarifsUniques: { pai: 3.97, occasionnel: 7.0, ext649: 7.0, ext650: 8.25 },
    mois: [
  ...
  schoolCalendar: {
    ...
    holidayDates: [
      ...
      {
        date: "2027-05-17",
        label: "Lundi de Pentecôte (50 jours après Pâques)",
      },
    ],
    months: {
...
async function handleLogin() {
  ...
  if (!email || !email.toLowerCase().endsWith("@laxou.fr")) {
    showAuthError(
      "Accès refusé : Seules les adresses e-mail se terminant par @laxou.fr sont autorisées.",
    );
    return;
  }
  ...
  if (password !== MASTER_ADMIN_PASSWORD) {
    showAuthError(
      "Mot de passe municipal incorrect. Veuillez saisir le mot de passe Mairie.",
    );
    return;
  }
...
async function loadConfig() {
  try {
    ...
    currentConfig =
      config && config.qfBrackets ? config : DEFAULT_FALLBACK_CONFIG;
...
function renderAllConfig(config) {
  if (config.anneeScolaireLabel)
    document.getElementById("anneeScolaireLabel").value =
      config.anneeScolaireLabel;
  if (config.zoneScolaire)
    document.getElementById("zoneScolaire").value = config.zoneScolaire;
...
function renderTranches(config) {
  ...
  if (document.getElementById("statTranchesCount")) {
    document.getElementById("statTranchesCount").textContent =
      qfBrackets.length;
  }
  ...
  tbody
    ...
    .forEach((input) => input.addEventListener("change", showSaveBar));
}
...
function addTranche() {
  ...
  currentConfig.qfBrackets.push({
    min: minVal,
    max: 999999,
    label: `${minVal} et plus`,
  });
  currentConfig.garderie.rates.push({ normal: 1.5, reduit: 1.2 });
  currentConfig.restauration.rates.push({ normal: 6.5, reduit: 5.0 });
...
function deleteTranche(idx) {
  ...
  if (currentConfig.garderie.rates[idx])
    currentConfig.garderie.rates.splice(idx, 1);
  if (currentConfig.restauration.rates[idx])
    currentConfig.restauration.rates.splice(idx, 1);
  renderTranches(currentConfig);
  showSaveBar();
}
...
function renderTarifsUniques(config) {
  if (config.garderie && config.garderie.tarifsUniques) {
    document.getElementById("tarifGarderieOccasionnel").value =
      config.garderie.tarifsUniques.occasionnel || 2.1;
    document.getElementById("tarifGarderieExterieur").value =
      config.garderie.tarifsUniques.exterieur || 1.66;
  }
  if (config.restauration && config.restauration.tarifsUniques) {
    document.getElementById("tarifRestaurationPai").value =
      config.restauration.tarifsUniques.pai || 3.97;
    document.getElementById("tarifRestaurationOccasionnel").value =
      config.restauration.tarifsUniques.occasionnel || 7.0;
    document.getElementById("tarifRestaurationExt649").value =
      config.restauration.tarifsUniques.ext649 || 7.0;
    document.getElementById("tarifRestaurationExt650").value =
      config.restauration.tarifsUniques.ext650 || 8.25;
  }

  document
    .querySelectorAll("#uniques input")
    .forEach((input) => input.addEventListener("change", showSaveBar));
}
...
function renderVacances(config) {
  ...
  const periods =
    (config.schoolCalendar && config.schoolCalendar.periods) || {};
...
function renderHolidays(config) {
  ...
  const holidays =
    (config.schoolCalendar && config.schoolCalendar.holidayDates) || [];
...
function addHoliday() {
  ...
  if (!currentConfig.schoolCalendar.holidayDates)
    currentConfig.schoolCalendar.holidayDates = [];
  currentConfig.schoolCalendar.holidayDates.push({
    date: "2027-05-20",
    label: "Jour férié local",
  });
...
function renderOccurrences(config) {
  ...
  const monthsMap =
    (config.schoolCalendar && config.schoolCalendar.months) || {};
  const gMois = (config.garderie && config.garderie.mois) || [];

  gMois.forEach((m) => {
    const occ = monthsMap[m.key] || {
      lundi: 4,
      mardi: 4,
      mercredi: 4,
      jeudi: 4,
  ...
  try {
    ...
    if (data.success) {
      ...
      if (data.statistiquesGlobales) {
        document.getElementById("statSimCount").textContent =
          data.statistiquesGlobales.totalSimulations || currentHistory.length;
        document.getElementById("statQfMoyen").textContent =
          data.statistiquesGlobales.qfMoyen || 650;
        document.getElementById("statMensuelMoyen").textContent =
          `${(data.statistiquesGlobales.mensuelMoyenFoyer || 106.0).toFixed(2)} €`;
      }
...
function renderHistoryRows(historyList) {
  ...
  historyList.forEach((sim) => {
    const dateFormatted = new Date(sim.date || Date.now()).toLocaleString(
      "fr-FR",
    ...
    const gTotal = sim.synthese
      ? sim.synthese.totalGarderie
      : sim.estimationGarderie || 0;
    const rTotal = sim.synthese
      ? sim.synthese.totalRestauration
      : sim.estimationRestauration || 0;
    const genTotal = sim.synthese
      ? sim.synthese.totalGeneral
      : sim.estimationMensuelle || 0;
...
function filterHistory() {
  ...
  const filtered = currentHistory.filter((sim) => {
    const prenoms =
      sim.family && sim.family.prenoms ? sim.family.prenoms.toLowerCase() : "";
    const qfStr = (sim.family ? sim.family.qf : "").toString();
    const dateStr = new Date(sim.date || Date.now())
      .toLocaleString("fr-FR")
      .toLowerCase();
    return (
      prenoms.includes(searchVal) ||
      qfStr.includes(searchVal) ||
...
async function deleteHistoryEntry(simId) {
  if (
    !confirm(
      "Voulez-vous vraiment supprimer cette simulation de l'historique ?",
    )
  ) {
    return;
  }
...
function exportToExcel() {
  ...
  let csvContent = "\uFEFF";
  csvContent +=
    "ID;Date & Heure;Prénoms / Nom Foyer;Quotient Familial (€);Nombre Enfants;Estimation Garderie (€);Estimation Cantine (€);Total General (€)\n";
...
async function saveConfiguration() {
  ...
  currentConfig.anneeScolaireLabel =
    document.getElementById("anneeScolaireLabel").value;
  ...
  trancheRows.forEach((tr, idx) => {
    ...
    if (inputs.length >= 6) {
      ...
      currentConfig.qfBrackets[idx].label =
        `${inputs[0].value} – ${inputs[1].value}`;

      currentConfig.garderie.rates[idx].normal =
        parseFloat(inputs[2].value) || 0;
      currentConfig.garderie.rates[idx].reduit =
        parseFloat(inputs[3].value) || 0;

      currentConfig.restauration.rates[idx].normal =
        parseFloat(inputs[4].value) || 0;
      currentConfig.restauration.rates[idx].reduit =
  ...
  if (currentConfig.garderie && currentConfig.garderie.tarifsUniques) {
    currentConfig.garderie.tarifsUniques.occasionnel =
      parseFloat(document.getElementById("tarifGarderieOccasionnel").value) ||
      2.1;
    currentConfig.garderie.tarifsUniques.exterieur =
      parseFloat(document.getElementById("tarifGarderieExterieur").value) ||
      1.66;
  }
  if (currentConfig.restauration && currentConfig.restauration.tarifsUniques) {
    currentConfig.restauration.tarifsUniques.pai =
      parseFloat(document.getElementById("tarifRestaurationPai").value) || 3.97;
    currentConfig.restauration.tarifsUniques.occasionnel =
      parseFloat(
        document.getElementById("tarifRestaurationOccasionnel").value,
      ) || 7.0;
    currentConfig.restauration.tarifsUniques.ext649 =
      parseFloat(document.getElementById("tarifRestaurationExt649").value) ||
      7.0;
    currentConfig.restauration.tarifsUniques.ext650 =
      parseFloat(document.getElementById("tarifRestaurationExt650").value) ||
      8.25;
  }
  ...
  plafondRows.forEach((tr, idx) => {
    ...
    if (currentConfig.garderie && currentConfig.garderie.mois[idx]) {
      currentConfig.garderie.mois[idx].matin =
        parseInt(inputs[0].value, 10) || 0;
      currentConfig.garderie.mois[idx].soir =
        parseInt(inputs[1].value, 10) || 0;
    }
    if (currentConfig.restauration && currentConfig.restauration.mois[idx]) {
      currentConfig.restauration.mois[idx].repas =
  ...
  vacInputs.forEach((input) => {
    ...
    if (
      currentConfig.schoolCalendar &&
      currentConfig.schoolCalendar.periods &&
      currentConfig.schoolCalendar.periods[key]
    ) {
  ...
  occRows.forEach((tr) => {
    ...
    if (inputs.length >= 5) {
      ...
      if (
        currentConfig.schoolCalendar &&
        currentConfig.schoolCalendar.months &&
        currentConfig.schoolCalendar.months[key]
      ) {
        currentConfig.schoolCalendar.months[key].lundi =
          parseInt(inputs[0].value, 10) || 0;
        currentConfig.schoolCalendar.months[key].mardi =
          parseInt(inputs[1].value, 10) || 0;
        currentConfig.schoolCalendar.months[key].mercredi =
          parseInt(inputs[2].value, 10) || 0;
        currentConfig.schoolCalendar.months[key].jeudi =
          parseInt(inputs[3].value, 10) || 0;
        currentConfig.schoolCalendar.months[key].vendredi =
  ...
  try {
    ...
    if (data.success) {
      alert(
        "✅ Toute la grille tarifaire, les plafonds et le calendrier ont été enregistrés avec succès.",
      );
      const saveBar = document.getElementById("saveBar");
