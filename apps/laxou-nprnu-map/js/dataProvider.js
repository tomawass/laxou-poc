...
export class DataProvider {
  ...
  async loadData(urlOrPath = './data.json') {
    ...
    if (!rawData || !rawData.places) {
      if (typeof window !== 'undefined' && (window.LAXOU_NPRNU_DATA || window.LAXOU_DATA)) {
  ...
  _persistCustomData() {
    if (typeof window !== 'undefined' && window.localStorage) {
      try {
        localStorage.setItem('LAXOU_CUSTOM_DATA_V2', JSON.stringify({
          places: this.places
        }));
      } catch (e) {
  ...
  validateSchema(rawData) {
    ...
    for (let i = 0; i < rawData.places.length; i++) {
      ...
      if (!hasLat && !hasXY) {
        throw new Error(`Invalid place "${p.id}": missing or invalid coordinates (lat/lng or x/y).`);
      }
    }
  }
