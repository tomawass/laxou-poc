...
describe("Adversarial Stress Test — Challenger 2 (Milestone 1)", () => {
  ...
  describe("1. data.json Schema & Domain Integrity", () => {
    ...
    it("should load data.json and verify root metadata & structure", async () => {
      ...
      assert.equal(
        ...
        5,
        "Must contain exactly 5 categories",
      );
    });
    ...
    it("should verify non-empty descriptions, tags, names, images, and addresses", () => {
      data.places.forEach((place) => {
        ...
        assert.ok(
          ...
          `Place ${place.id} missing name`,
        );
        assert.ok(
          typeof place.address === "string" && place.address.trim().length > 0,
          `Place ${place.id} missing address`,
        );
        assert.ok(
          typeof place.description === "string" &&
            place.description.trim().length >= 10,
          `Place ${place.id} description too short`,
        );
        assert.ok(
          typeof place.image === "string" && place.image.startsWith("http"),
          `Place ${place.id} missing valid image URL`,
        );
        assert.ok(
          Array.isArray(place.tags) && place.tags.length >= 2,
          `Place ${place.id} tags array must have at least 2 elements`,
        );

        place.tags.forEach((tag) => {
          assert.ok(
            typeof tag === "string" && tag.trim().length > 0,
            `Place ${place.id} has empty tag element`,
          );
        });
      });
    });
    ...
    it("should verify NPRNU Champ-le-Bœuf coverage (at least 4 NPRNU POIs in Champ-le-Bœuf)", () => {
      ...
      assert.ok(
        nprnuPlaces.length >= 5,
        `Expected at least 5 NPRNU POIs overall, found ${nprnuPlaces.length}`,
      );

      const champLeBoeufNprnu = nprnuPlaces.filter((p) => {
        const text =
          `${p.name} ${p.description} ${p.address} ${(p.tags || []).join(" ")}`.toLowerCase();
  ...
  describe("2. DataProvider Diacritics & Ligatures Edge Cases", () => {
    ...
    it('should test ligature "oe" vs "œ" equivalence in search queries', () => {
      ...
      assert.ok(
        query1.length >= 4,
        `Query "boeuf" should return at least 4 POIs, got ${query1.length}`,
      );
      assert.equal(
        query1.length,
        query2.length,
        '"boeuf" vs "bœuf" should return exact same count',
      );
      assert.equal(
        query1.length,
        query3.length,
        '"boeuf" vs "BOEUF" should return exact same count',
      );
      assert.equal(
        query1.length,
        query4.length,
      ...
      assert.deepEqual(
        ids1,
        ids2,
        'Matched POI IDs must be identical for "boeuf" and "bœuf"',
      );
    });
    ...
    it('should test diacritics "e" vs "é/è/ê" and "a" vs "à/â"', () => {
      ...
      assert.ok(
        queryEco1.length >= 2,
        `Query "ecole" should match at least 2 items`,
      );
      assert.equal(
        queryEco1.length,
        queryEco2.length,
        '"ecole" vs "école" match count mismatch',
      );
      assert.equal(
        queryEco1.length,
        queryEco3.length,
    ...
    it("should test partial word match and address field searching", () => {
      ...
      assert.ok(
        addressSearch1.length >= 2,
        "Should find Hôtel de Ville & Opéra via Stanislas address",
      );
      ...
      assert.equal(
        addressSearch2.length,
        2,
        "Should find Nancy Thermal and Lycée Chopin on Sergent Blandan",
      );
    });
    ...
    it("should handle category filtering combined with search query", () => {
      ...
      const filteredNone = dp.filterPlaces(
        "culture",
  ...
  describe("3. Projection Math, Linearity & Metric Distortion", () => {
    it("should verify strict linearity of geoToWorld mapping", () => {
      ...
      for (let i = 0; i <= 50; i++) {
        ...
        assert.ok(
          Math.abs(w.x - expectedX) < 1e-12,
          `x linearity failed at t=${t}`,
        );
        assert.ok(
          Math.abs(w.y - expectedY) < 1e-12,
          `y linearity failed at t=${t}`,
        );
      }
    });
    ...
    it("should verify screenToGeo and geoToScreen roundtrip identity across multi-scale viewports", () => {
      ...
      const testPoints = [
        ...
        { lat: 48.675, lng: 6.135 },
        { lat: 48.702, lng: 6.195 },
    ...
    it("should verify metric distortion and cosine latitude scaling factor", () => {
      ...
      const expectedGeoAR =
        ((bounds.maxLng - bounds.minLng) * expectedCos) /
    ...
    it("should handle zero-span and inverted bounds defensively", () => {
      ...
      assert.ok(
        isFinite(worldZero.x) && isFinite(worldZero.y),
        "Zero-span should not yield NaN",
      );
      ...
      assert.ok(
        proj.bounds.minLat < proj.bounds.maxLat,
        "Inverted bounds should be swapped",
      );
      assert.ok(
        proj.bounds.minLng < proj.bounds.maxLng,
        "Inverted bounds should be swapped",
      );
    });
