...
test("Milestone 3 — MarkerManager Component", async (t) => {
  ...
  const projection = new Projection(Projection.DEFAULT_BOUNDS, {
    width: 800,
    height: 600,
  });
  const viewport = new ViewportController(projection, eventBus, {
    width: 800,
    height: 600,
  });
  ...
  await t.test(
    "should update marker positions on viewport:changed event",
    () => {
      let callCount = 0;
      ...
      assert.ok(
        callCount > 0,
        "updatePositions should be triggered by viewport:changed",
      );
    },
  );

  await t.test(
    "should activate correct marker when place:selected is emitted",
    () => {
  ...
  await t.test(
    "should filter marker visibility with setVisiblePlaces()",
    () => {
...
test("Milestone 3 — SidebarController Component", async (t) => {
  ...
  await t.test(
    "should hide detail drawer when hideDetailDrawer() is called",
    () => {
