...
export class SidebarController {
  ...
  _createPlaceCard(place, categories) {
    ...
    const catColor =
      SidebarController.DEFAULT_CATEGORY_HIGHLIGHTS[place.category] ||
      "#3b82f6";
    ...
    const highlightColor =
      place.highlightColor ||
      SidebarController.DEFAULT_CATEGORY_HIGHLIGHTS[place.category] ||
      "#d8f0c1";
    const fallbackSvg = this._getCategoryFallbackSvg(
      place.category,
      place.name,
    );
    ...
    if (statusText === "En cours") {
      statusBadgeHtml =
        '<span class="status-badge-active status-encours">En cours</span>';
    } else if (statusText === "À venir") {
      statusBadgeHtml =
        '<span class="status-badge-active status-avenir">À venir</span>';
    } else if (statusText === "Livré") {
      statusBadgeHtml =
        '<span class="status-badge-active status-livre">Livré</span>';
  ...
  _formatRichContent(text) {
    ...
    lines.forEach((line) => {
      ...
      if (trimmed.startsWith("•") || trimmed.startsWith("-")) {
        ...
        const formattedLine = content.replace(
          /^([^:]+):/,
          "<strong>$1 :</strong>",
        );
  ...
  _getCategoryFallbackSvg(category, name) {
    const configs = {
      ...
      parcs: {
        ...
        bg2: "#15803d",
        label: "Parc & Nature",
        icon: "M12 2L4.5 13h4v8h7v-8h4L12 2z",
      },
      services: {
        bg1: "#2563eb",
        bg2: "#1d4ed8",
        label: "Service & Aménagement",
      ...
      sports: {
        ...
        bg2: "#c2410c",
        label: "Sport & Santé",
        icon: "M20.57 14.86L18 12.29l1.43-1.43",
      },
      mobilites: {
        bg1: "#9333ea",
        bg2: "#7e22ce",
        label: "Mobilité & Connexion",
        icon: "M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13",
      },
    };
    const cfg = configs[category] || configs.services;
    const cleanName = (name || "Laxou Aménagement")
      .replace(/</g, "&lt;")
      .replace(/>/g, "&gt;");
