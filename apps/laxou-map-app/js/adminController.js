...
export class AdminController {
  ...
  showEditorModal(place = null) {
    ...
    document.getElementById('edit-place-lat').value = place ? place.lat : 48.6865;
    document.getElementById('edit-place-lng').value = place ? place.lng : 6.1504;
    document.getElementById('edit-place-address').value = place ? place.address : 'Laxou (54520)';
    document.getElementById('edit-place-desc').value = place ? place.description : '';
    document.getElementById('edit-place-image').value = place ? (place.image || '') : '';
    document.getElementById('edit-place-phone').value = place ? (place.phone || '') : '';
    document.getElementById('edit-place-email').value = place ? (place.email || '') : '';
    document.getElementById('edit-place-link').value = place ? (place.link || '') : '';
    document.getElementById('edit-place-nprnu').checked = place ? Boolean(place.isNprnu) : false;
  ...
  savePlaceFromForm() {
    ...
    if (!name || !isFinite(lat) || !isFinite(lng)) {
      alert('Veuillez remplir au moins le nom et les coordonnées géographiques valides.');
      return;
    }
    ...
    if (this.editingPlaceId) {
      ...
      this.dataProvider.updatePlace(this.editingPlaceId, {
        ...
        link,
        isNprnu,
        tags: [category.capitalize ? category.capitalize() : category, "Laxou", name.split(' ')[0]]
      });
    } else {
      // Création
      ...
      this.dataProvider.addPlace({
        ...
        image: image || 'https://images.unsplash.com/photo-1541872703-74c5e44368f9?w=600&auto=format&fit=crop',
        phone,
        email,
        link,
        isNprnu,
