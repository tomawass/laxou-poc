...
describe('Milestone 1 — EventBus Component', () => {
  ...
  it('should clear all listeners with clear()', () => {
    ...
    bus.on('ev1', () => { called = true; });
    bus.on('ev2', () => { called = true; });
...
describe('Milestone 1 — Projection Engine', () => {
  ...
  it('should preserve bidirectional roundtrip identity (geoToScreen -> screenToGeo)', () => {
    ...
    const testCoords = [
      ...
      { lat: 48.6790, lng: 6.1688 }  // Nancy Thermal
    ];
    ...
    for (const vp of viewports) {
      for (const coord of testCoords) {
        ...
        assert.ok(Math.abs(restoredGeo.lat - coord.lat) < 1e-9, `Lat roundtrip failed for ${coord.lat}`);
        assert.ok(Math.abs(restoredGeo.lng - coord.lng) < 1e-9, `Lng roundtrip failed for ${coord.lng}`);
      }
    }
  });
  ...
  it('should verify isPointInBounds helper', () => {
    ...
    assert.equal(proj.isPointInBounds(40.0000, 2.0000), false);
  });
});
...
describe('Milestone 1 — Data Model & data.json Integrity', () => {
  it('should load data.json and verify POIs across 5 categories', async () => {
    ...
    assert.deepEqual(categoryIds, ['culture', 'ecoles', 'parcs', 'services', 'sports']);
  });
  ...
  it('should verify that all POIs contain required schema fields', async () => {
    ...
    const requiredFields = ['id', 'name', 'category', 'lat', 'lng', 'address', 'description', 'image', 'tags', 'link'];
    ...
    for (const place of places) {
      ...
      assert.ok(Array.isArray(place.tags), `POI "${place.id}" tags must be an array`);
      assert.ok(isFinite(place.lat) && isFinite(place.lng), `POI "${place.id}" lat/lng must be numeric`);
    }
  });
  ...
  it('should verify that NPRNU locations exist in Laxou (Champ-le-Bœuf & Provinces)', async () => {
    ...
    const nprnuPlaces = places.filter(p => p.isNprnu === true || p.tags.includes('NPRNU'));
...
describe('Milestone 1 — DataProvider Filtering & Accent-Insensitive Search', () => {
  ...
  it('should throw an error when loading invalid dataset schema', () => {
    ...
    assert.throws(() => {
      invalidProvider.validateSchema({ places: 'not-an-array', categories: [] });
    }, /places/);
