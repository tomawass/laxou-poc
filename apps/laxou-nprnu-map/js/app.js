...
export class App {
  ...
  _initEngineComponents() {
    const { mapView, mapImageContainer, markerOverlay, zoomInBtn, zoomOutBtn } =
      this.elements;

    // 1. Moteur de carte sur image
    if (mapView && mapImageContainer) {
      this.imageEngine = new ImageMapEngine(
        mapView,
        mapImageContainer,
        this.eventBus,
      );
  ...
  setupEventListeners() {
    const { toggleSidebarBtn, closeSidebarBtn, sidebar, themeToggleBtn } =
      this.elements;
    ...
    function getYouTubeEmbedUrl(url) {
      ...
      const startMatch =
        url.match(/[?&]t=(\d+)s?/) || url.match(/[?&]start=(\d+)/);
      const startParam = startMatch
        ? `?start=${startMatch[1]}&autoplay=1`
        : "?autoplay=1";
    ...
    const renderVideoModal = (selectedIdx = 0) => {
      ...
      if (videos.length === 0) {
        dualVideoCards.innerHTML =
          '<p style="color: #64748b; font-size: 0.9rem;">Aucune vidéo disponible pour le moment.</p>';
    ...
    const closeAdminVideoModal = document.getElementById(
      "close-admin-video-modal",
    );
    ...
    const renderAdminVideosList = () => {
      ...
      if (videos.length === 0) {
        adminVideosList.innerHTML =
          '<p style="color: #64748b; font-size: 0.85rem;">Aucune vidéo enregistrée.</p>';
        return;
      }
      ...
      videos.forEach((vid, idx) => {
        ...
        item.style.cssText =
          "display: flex; align-items: center; justify-content: space-between; padding: 0.6rem 0.8rem; background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 10px; font-size: 0.88rem;";
        item.innerHTML = `
          <div>
    ...
    const closeAdminLoginModalBtn = document.getElementById(
    ...
    if (this.elements.mapImageContainer) {
      this.elements.mapImageContainer.addEventListener("click", (e) => {
        if (
          ...
          const xPct = Math.max(
            0,
            Math.min(100, parseFloat(((clickX / rect.width) * 100).toFixed(1))),
          );
          const yPct = Math.max(
            0,
            Math.min(
              100,
              parseFloat(((clickY / rect.height) * 100).toFixed(1)),
            ),
          );
    ...
    const closeAdminEditorModalBtn = document.getElementById(
      "close-admin-editor-modal",
    );
