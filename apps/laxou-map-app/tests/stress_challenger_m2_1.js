...
describe("Adversarial Stress Suite — ViewportController (Challenger 1, M2)", () => {
  ...
  describe("1. Camera Bounds Clamping with Extreme Values", () => {
    it("1.1 should handle extreme positive and negative panBy values (Infinity, 1e30, -1e30)", () => {
      ...
      assert.ok(
        !Number.isNaN(vp.x) && !Number.isNaN(vp.y),
    ...
    it("1.2 should handle malformed panBy inputs (NaN, null, undefined, strings, objects)", () => {
      ...
      for (const [dx, dy] of malformedInputs) {
        ...
        assert.ok(
          isFinite(vp.x),
          `vp.x corrupt on [${String(dx)}, ${String(dy)}]: ${vp.x}`,
        );
        assert.ok(
          isFinite(vp.y),
          `vp.y corrupt on [${String(dx)}, ${String(dy)}]: ${vp.y}`,
        );
      }
    });
    ...
    it("1.4 should handle extreme setDimensions calls (0x0, negative, NaN, ultra-huge)", () => {
      ...
      for (const [w, h] of invalidDims) {
        ...
        assert.ok(
          vp.width > 0,
          `width must remain positive for input [${w}, ${h}]`,
        );
        assert.ok(
          vp.height > 0,
          `height must remain positive for input [${w}, ${h}]`,
        );
  ...
  describe("2. Zoom Levels at Boundaries & Floating Point Edge Cases", () => {
    it("2.1 should strictly clamp zoom to [minZoom, maxZoom] under extreme zoomAt multipliers", () => {
      ...
      assert.equal(
        vp.zoom,
        10.0,
        `Zoom should be capped at maxZoom 10.0, got ${vp.zoom}`,
      );
      ...
      assert.equal(
        vp.zoom,
        1.0,
        `Zoom should be floored at minZoom 1.0, got ${vp.zoom}`,
      );
    ...
    it("2.2 should clamp out-of-bounds zoom in centerOnGeo and fitBounds", () => {
      ...
      assert.equal(
        vp.zoom,
        1.0,
        `centerOnGeo zoom level 0.2 should clamp to 1.0, got ${vp.zoom}`,
      );
      ...
      assert.equal(
        vp.zoom,
        10.0,
        `centerOnGeo zoom level 25.0 should clamp to 10.0, got ${vp.zoom}`,
      );
      ...
      assert.equal(
        vp.zoom,
        10.0,
        "centerOnGeo with NaN zoom should preserve previous zoom",
      );
    });
    ...
    it("2.4 should honor epsilon zoom threshold and prevent event emission on negligible zoom delta", () => {
      ...
      const proj = new Projection();
      const vp = new ViewportController(proj, bus, {
        width: 800,
        height: 600,
        zoom: 2.0,
      });
      ...
      assert.equal(
        emitCount,
        0,
        "No viewport:changed event should be emitted for sub-epsilon zoom delta",
      );
      ...
      assert.equal(
        emitCount,
        1,
        "Event should be emitted for significant zoom delta",
      );
    });
  ...
  describe("4. Gesture Pan State Transitions & Double-Click / Button Zoom", () => {
    ...
    it("4.4 should correctly scale wheel delta across deltaMode 0 (pixel), 1 (line), and 2 (page)", () => {
      ...
      vp.zoom = 2.0;
      mockContainer.dispatchMockEvent("wheel", {
        clientX: 400,
        clientY: 300,
        deltaY: 100,
        deltaMode: 0,
      ...
      assert.ok(
        zoomPixel < 2.0,
        `Wheel scroll down should zoom out: got ${zoomPixel}`,
      );
      ...
      assert.ok(
        zoomLine < 2.0,
        `Wheel line mode scroll should zoom out: got ${zoomLine}`,
      );
      ...
      assert.ok(
        zoomPage < 2.0,
        `Wheel page mode scroll should zoom out: got ${zoomPage}`,
      );
    });
    ...
    it("4.5 should handle double-click zoom (dblclick event) and zoom control buttons", () => {
      ...
      mockContainer.dispatchMockEvent("dblclick", {
        clientX: 450,
        clientY: 350,
      });
      assert.equal(
        vp.zoom,
        3.0,
        `Double click should increase zoom 1.5x from 2.0 to 3.0, got ${vp.zoom}`,
      );
      ...
      assert.equal(
        vp.zoom,
        3.75,
        `Zoom in button should increase zoom by 1.25x from 3.0 to 3.75, got ${vp.zoom}`,
      );
      ...
      assert.equal(
        vp.zoom,
        3.0,
        `Zoom out button should decrease zoom by 0.8x from 3.75 to 3.0, got ${vp.zoom}`,
      );
    });
  });
});
