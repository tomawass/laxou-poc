            ...
            navItems.forEach((sibling) => {
              if (sibling !== item) {
                const siblingDropdown = sibling.querySelector(
                  ".dropdown-menu, .mega-menu-3col",
                );
                if (siblingDropdown)
                  siblingDropdown.classList.remove("is-open");
...
function initScrollReveal() {
  ...
  const grids = document.querySelectorAll(
    ".acces-rapides-grid, .grid-posters, .grid-4, .grid-3-projects",
  );
  grids.forEach((grid) => {
...
function initGdprForms() {
  ...
  forms.forEach((form) => {
    form.addEventListener("submit", (e) => {
      e.preventDefault();
      alert(
        "Merci ! Votre demande a été enregistrée avec succès par les services de la Mairie de Laxou.",
      );
      form.reset();
    });
  });
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const routes = [
    {
      ...
      url: "simulateur-periscolaire.html",
    },
    {
      title: "Carte Interactive (Projets & NPRNU)",
    ...
    {
      title: "Équipe Municipale & Élus",
      icon: "fa-users",
      keys: ["elu", "elus", "maire", "garcia", "adjoint", "conseil", "equipe"],
      url: "equipe-municipale.html",
    },
    {
      title: "Démarches : État Civil & CNI",
      icon: "fa-id-card",
      keys: [
        "passeport",
        "cni",
    ...
    },
    {
      ...
      icon: "fa-volleyball",
      keys: ["sport", "gymnase", "piscine", "terrain", "stade", "complexe"],
      url: "iframe.html",
    },
  ];
  ...
  function renderResults(query) {
    ...
    const results = routes.filter((route) =>
      route.keys.some((k) => k.includes(q) || q.includes(k)),
    );

    if (results.length === 0) {
      dropdown.innerHTML =
        '<div class="autocomplete-item" style="color: #888;">Aucun résultat trouvé...</div>';
