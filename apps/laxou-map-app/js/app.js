...
export class App {
  ...
  _initEngineComponents() {
    const { mapView, mapCanvas, markerOverlay, zoomInBtn, zoomOutBtn } =
      this.elements;
    ...
    // 1. Projection (M1)
    this.projection = new Projection(Projection.DEFAULT_BOUNDS, {
      width,
      height,
    });

    // 2. Viewport Controller (M2)
    this.viewport = new ViewportController(this.projection, this.eventBus, {
      width,
      height,
    });
    ...
    this.eventBus.on("place:selected", ({ placeId, place }) => {
      ...
      if (
        targetPlace &&
        isFinite(targetPlace.lat) &&
        isFinite(targetPlace.lng)
      ) {
        if (this.leafletEngine) {
          this.leafletEngine.centerOnGeo(targetPlace.lat, targetPlace.lng);
        } else if (this.viewport) {
          this.viewport.centerOnGeo(
            targetPlace.lat,
            targetPlace.lng,
            Math.max(3.0, this.viewport.zoom),
          );
        }
      }
    });
  ...
  async loadData() {
    try {
      ...
      if (appData.metadata && appData.metadata.center) {
        ...
        if (this.leafletEngine) {
          this.leafletEngine.centerOnGeo(
            c.lat,
            c.lng,
            appData.metadata.defaultZoom || 14,
          );
        } else if (this.viewport) {
          this.viewport.centerOnGeo(
            c.lat,
            c.lng,
            appData.metadata.defaultZoom || 1.0,
          );
        }
      }
    ...
    if (adminLoginBtn) {
      adminLoginBtn.addEventListener("click", () => {
        if (this.adminController.isAdmin) {
          alert(
            "Vous êtes actuellement connecté en tant qu'administrateur (@laxou.fr).",
          );
        } else {
