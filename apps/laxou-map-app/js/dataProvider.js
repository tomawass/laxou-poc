...
export class DataProvider {
  ...
  async loadData(urlOrPath = './data.json') {
    ...
    if (typeof urlOrPath === 'object' && urlOrPath !== null) {
      ...
      try {
        ...
        throw new Error(`Failed to load data file in Node.js environment from "${urlOrPath}": ${err.message}`);
      }
    } else {
      // Browser environment
      try {
        ...
        if (typeof window !== 'undefined' && window.LAXOU_DATA) {
          ...
          throw new Error(`Failed to fetch data from "${urlOrPath}": ${err.message}`);
        }
      }
    }
  ...
  _persistCustomData() {
    if (typeof window !== 'undefined' && window.localStorage) {
      try {
        localStorage.setItem('LAXOU_CUSTOM_DATA', JSON.stringify({
          places: this.places
        }));
      } catch (e) {
  ...
  validateSchema(rawData) {
    ...
    for (let i = 0; i < rawData.places.length; i++) {
      ...
      if (!p.id || typeof p.id !== 'string') {
        throw new Error(`Invalid place at index ${i}: missing or invalid "id".`);
      }
      if (!p.name || typeof p.name !== 'string') {
        throw new Error(`Invalid place "${p.id}": missing or invalid "name".`);
      }
      if (!p.category || typeof p.category !== 'string') {
        throw new Error(`Invalid place "${p.id}": missing or invalid "category".`);
      }
      if (!isFinite(Number(p.lat)) || !isFinite(Number(p.lng))) {
        throw new Error(`Invalid place "${p.id}": missing or invalid lat/lng coordinates.`);
      }
    }
  }
  ...
  filterPlaces(categoryId = 'all', searchQuery = '') {
    const normCategory = (categoryId && typeof categoryId === 'string') ? categoryId.trim() : 'all';
    const normQuery = this.normalizeText(searchQuery).trim();

    return this.places.filter(place => {
      // Category filter check
      if (normCategory !== 'all' && normCategory !== '' && place.category !== normCategory) {
        return false;
      }
      ...
      if (normQuery.length > 0) {
        ...
        if (Array.isArray(place.tags)) {
          tagsNormMatch = place.tags.some(tag => this.normalizeText(tag).includes(normQuery));
        }
