...
describe('Challenger 2 — CanvasEngine & Viewport Stress Test Suite', () => {
  ...
  describe('1. Rapid Viewport Pan & Zoom Stability', () => {
    it('should handle 1,000 rapid panBy calls without coordinate drift or NaN', () => {
      ...
      assert.ok(isFinite(state.zoom), `state.zoom must be finite, got ${state.zoom}`);
    ...
    it('should handle rapid alternating zoomAt operations with varying focal points', () => {
      ...
      const state = vp.getState();
      assert.ok(state.zoom >= 1.0 && state.zoom <= 10.0, `Zoom ${state.zoom} out of bounds [1, 10]`);
    ...
    it('should coalesce dirty-flag requestRedraw() calls when requestAnimationFrame is available', () => {
      ...
      assert.equal(renderCount, 0, 'render() should not be called synchronously when rAF is pending');

      // Now fire animation frame callback
      pendingCb();

      assert.equal(renderCount, 1, `Expected exactly 1 render call, got ${renderCount}`);
      assert.equal(engine.needsRedraw, false, 'needsRedraw should be reset after render');
  ...
  describe('2. Canvas Resizing & High/Low DPR Values', () => {
    it('should scale canvas buffer correctly for High DPR=3.0 (Retina Display)', () => {
      ...
      assert.equal(canvas.width, 2400, `Canvas buffer width should be 800 * 3 = 2400, got ${canvas.width}`);
      assert.equal(canvas.height, 1800, `Canvas buffer height should be 600 * 3 = 1800, got ${canvas.height}`);
      assert.equal(canvas.style.width, '800px');
      assert.equal(canvas.style.height, '600px');

      if (originalDPR !== undefined) global.window.devicePixelRatio = originalDPR;
    ...
    it('should scale canvas buffer correctly for Low DPR=0.5', () => {
      ...
      assert.equal(canvas.width, 400, `Canvas buffer width should be 800 * 0.5 = 400, got ${canvas.width}`);
      assert.equal(canvas.height, 300, `Canvas buffer height should be 600 * 0.5 = 300, got ${canvas.height}`);
      assert.equal(canvas.style.width, '800px');
      assert.equal(canvas.style.height, '600px');
    ...
    it('should check if CanvasEngine updates canvas DOM buffer size on container resize via viewport:changed', () => {
      ...
      canvas.getBoundingClientRect = () => ({ left: 0, top: 0, width: 1200, height: 900 });
      ...
      assert.equal(engine.cssWidth, 1200, `CanvasEngine.cssWidth should be updated to 1200 on container resize, got ${engine.cssWidth}`);
      assert.equal(canvas.width, 1200, `canvas.width buffer should be updated to 1200 on container resize, got ${canvas.width}`);
    });
  });
  ...
  describe('3. Dynamic Metric Scale Bar Calculations', () => {
    it('should compute valid scale bar pixel widths and metric step values across all zoom levels z=1..10', () => {
      ...
      zoomLevels.forEach((zoom) => {
        ...
        const scaleText = textCalls.find(c => typeof c.args[0] === 'string' && (c.args[0].endsWith('m') || c.args[0].endsWith('km')));

        assert.ok(scaleText, `Scale bar text missing at zoom level ${zoom}`);
        const label = scaleText.args[0];
        assert.ok(label.match(/^\d+(\.\d+)?\s*(m|km)$/), `Scale bar text format invalid: "${label}"`);
      });
    });
    ...
    it('should accurately match physical pixel distance to calculated meters per pixel', () => {
      ...
      assert.ok(metersPerPx > 5.0 && metersPerPx < 12.0, `Calculated meters per px ${metersPerPx} out of expected range for Laxou at z=1`);
    });
  });
  ...
  describe('4. Canvas Context Lifecycle & Exception Resilience', () => {
    it('should maintain balanced ctx.save() and ctx.restore() depth across render frames', () => {
      ...
      engine.render();
      assert.equal(ctx.getSaveDepth(), 0, `Context save depth must be 0 after render, got ${ctx.getSaveDepth()}`);

      engine.render();
      assert.equal(ctx.getSaveDepth(), 0, `Context save depth must be 0 after second render pass, got ${ctx.getSaveDepth()}`);
    });
    ...
    it('should render all vector layers (background, grid, districts, parks, waterways, roads, scalebar) cleanly', () => {
      ...
      assert.ok(lineToCalls.length > 50, `Expected >50 lineTo calls for vector geometry, got ${lineToCalls.length}`);
      assert.ok(strokeCalls.length > 5, `Expected >5 stroke calls, got ${strokeCalls.length}`);
      assert.ok(fillCalls.length >= 4, `Expected at least 4 district/park fill calls, got ${fillCalls.length}`);
    });
  });
});
